...
<div>
    <table>
        <tr>
            <td>
                PR <a href="https://github.com/hyperledger/firefly/pull/658" class=".btn">#658</a>
            </td>
            <td>
                <b>
        ...
        <tr>
            ...
            <td>
                This PR includes some significant changes to the way we automatically generate the OpenAPI Spec in FireFly.

## Input masking
- The old InputMask is totally gone. We now use tags on individual fields to indicate how to build the correct JSON Schema for a field.
- `ffexclude:"true"` can be used to always exclude a field from JSON Schema
- `ffexclude:"routeName,otherRouteName"` can be used to exclude a field from JSON Schema on routes named `routeName` or `otherRouteName`. It is case-sensitive and comma separated
- `ffexcludeinput:"true"` can be used to to always exclude a field from the **input** JSON Schema
- `ffexcludeinput:"routeName,otherRouteName"` can be used to exclude a field from the **input** JSON Schema on routes named `routeName` or `otherRouteName`. It is case-sensitive and comma separated

## Field level descriptions

Field level descriptions can be added in `internal/i18n/en_translations_descriptions.go`. Each struct field that wants to use one of these needs to have an ffstruct tag on it, indicating the name of the struct. That will be combined with the JSON field name (note, it is not the GO field name, but the JSON serialized name), separated by a "." This is the key used to lookup the translation below. If it is not found, the description is left blank in the OpenAPI spec.
...
## Other notes, or points worthy of discussion / further exploration:

- I have removed all "handcrafted" JSON schemas. I did not go so far as to remove the input and output schema fields from the Route struct, because the FFI autogeneration still uses them, as it basically just passes through the JSON Schema from the FFI. I may have broken this at the moment though - need to check that.
- It looks like the CSS is "weird" on the Swagger UI. The `description` field doesn't appear to be formatted consistently depending on what level the field is nested
- @awrichar requested the ability to mark fields as `required` in the JSON Schema. I think this is a great idea, but still need to work out how to add it, because the `required` field is a list of field names that go onto the _parent_ JSON Schema, which we don't currently have a pointer to at the time of field level customization.

## Updated kin-openapi library
Right now this PR will require you to clone the `master` branch of https://github.com/getkin/kin-openapi, as there was a change just merged today that it needs, and it hasn't had a new release yet. Just clone it into a directory that sits next to your `firefly` directory and it should be good to go.
            </td>
        </tr>
    </table>
    <div class="right-align">
        Created At 2022-04-01 18:16:09 +0000 UTC
    </div>
</div>
...
<div>
    <table>
        <tr>
            <td>
                PR <a href="https://github.com/hyperledger/firefly/pull/657" class=".btn">#657</a>
            </td>
            <td>
                <b>
                    Avoid blocking event processing threads when queuing rewinds
                </b>
            </td>
        </tr>
        <tr>
            ...
            <td>
                In PR chain with #656 
            </td>
        </tr>
    </table>
    <div class="right-align">
        Created At 2022-04-01 13:29:04 +0000 UTC
    </div>
</div>
...
<div>
    <table>
        <tr>
            <td>
                PR <a href="https://github.com/hyperledger/firefly/pull/656" class=".btn">#656</a>
            </td>
            <td>
                <b>
                    Move DX events interface to async with callback
                </b>
            </td>
        </tr>
        <tr>
            ...
            <td>
                Update FireFly Core to exploit the parallelism enabled by https://github.com/hyperledger/firefly-dataexchange-https/pull/56
...
- Updated manifest to pick up EthConnect `v3.1.6`
    ...
    <div class="right-align">
        Created At 2022-03-31 22:07:00 +0000 UTC
    </div>
</div>
...
<div>
    <table>
        <tr>
            <td>
                PR <a href="https://github.com/hyperledger/firefly/pull/655" class=".btn">#655</a>
            </td>
            <td>
                <b>
                    [chart-test-fix] chart testing fix
                </b>
            </td>
        </tr>
        <tr>
    ...
    <div class="right-align">
        Created At 2022-03-31 15:38:55 +0000 UTC
    </div>
</div>
...
<div>
    <table>
        <tr>
            <td>
                PR <a href="https://github.com/hyperledger/firefly/pull/654" class=".btn">#654</a>
            </td>
            <td>
                <b>
                    Update manifest.json and docker release build github action
                </b>
            </td>
        </tr>
        <tr>
    ...
    <div class="right-align">
        Created At 2022-03-31 15:05:16 +0000 UTC
    </div>
</div>
...
<div>
    <table>
        <tr>
            <td>
                PR <a href="https://github.com/hyperledger/firefly/pull/653" class=".btn">#653</a>
            </td>
            <td>
                <b>
...
However, the results clearly indicate to me it's only helping with lookup in the `message_id` based lookups - and not the `data_id` based lookups. So this PR proposes we replace it with two separate and simple indexes.
    ...
    <div class="right-align">
        Created At 2022-03-31 13:10:54 +0000 UTC
    </div>
</div>

---
layout: default
title: orion-server
parent: Hyperledger Labs
grand_parent: Pull Requests
has_children: false
permalink: /pull-requests/hyperledger-labs/orion-server
---

# orion-server <span class="fs-3 right-align">[GitHub](https://github.com/hyperledger-labs/orion-server){: .btn .mr-4 }</span>


<div>
    <table>
        <tr>
            <td>
                PR <a href="https://github.com/hyperledger-labs/orion-server/pull/340" class=".btn">#340</a>
            </td>
            <td>
                <b>
                    Data race in queue tests
                </b>
            </td>
        </tr>
        <tr>
            <td>
                <span class="chip">bug</span><span class="chip">testing</span>
            </td>
            <td>
                Cannot access the front of the promise with two different go-routine.

Signed-off-by: Yoav Tock <tock@il.ibm.com>
            </td>
        </tr>
    </table>
    <div class="right-align">
        Created At 2022-02-01 06:54:24 +0000 UTC
    </div>
</div>

<div>
    <table>
        <tr>
            <td>
                PR <a href="https://github.com/hyperledger-labs/orion-server/pull/331" class=".btn">#331</a>
            </td>
            <td>
                <b>
                    Data race in replication unit tests
                </b>
            </td>
        </tr>
        <tr>
            <td>
                <span class="chip">bug</span><span class="chip">replication</span><span class="chip">testing</span>
            </td>
            <td>
                Signed-off-by: Yoav Tock <tock@il.ibm.com>
            </td>
        </tr>
    </table>
    <div class="right-align">
        Created At 2022-01-30 13:43:47 +0000 UTC
    </div>
</div>

<div>
    <table>
        <tr>
            <td>
                PR <a href="https://github.com/hyperledger-labs/orion-server/pull/329" class=".btn">#329</a>
            </td>
            <td>
                <b>
                    New leader with in-flight blocks
                </b>
            </td>
        </tr>
        <tr>
            <td>
                <span class="chip">replication</span>
            </td>
            <td>
                When a new leader is elected, we have to prevent new proposals until it commits all the in-flight blocks.
            </td>
        </tr>
    </table>
    <div class="right-align">
        Created At 2022-01-27 13:17:33 +0000 UTC
    </div>
</div>

<div>
    <table>
        <tr>
            <td>
                PR <a href="https://github.com/hyperledger-labs/orion-server/pull/328" class=".btn">#328</a>
            </td>
            <td>
                <b>
                    Simple kubernetes deployment
                </b>
            </td>
        </tr>
        <tr>
            <td>
                
            </td>
            <td>
                One deployment
One service
Three persistent volumes

Signed-off-by: Gennady Laventman <gennady@il.ibm.com>
            </td>
        </tr>
    </table>
    <div class="right-align">
        Created At 2022-01-27 10:22:57 +0000 UTC
    </div>
</div>

